...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if constants.is_admin(username):
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group and is_mentioned and (match_en or match_ru):
            ...
            config = constants.get_config()
            messages_for_api = [{"role": "system", "content": config["SYSTEM_PROMPT"]}] + user_histories[user_id]
...
async def call_openrouter(messages: list) -> str:
    ...
    try:
        ...
        try:
            ...
            raise Exception(f"Both primary and fallback models failed. Primary error: {e}")
